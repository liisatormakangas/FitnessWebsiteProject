...
if (isLoggedIn) {
    ...
    fetch(`${shoppingCartUrl}?id_user=${userId}`)
        ...
        const courseDetailsPromises = cartItems.map((cartItem) => {
...
const renderCartItem = (course, cartItem) => {
...
const renderShoppingCart = (courses) => {
    //it may cause issue 
    ...
    courses.forEach((course) => {
        ...
        removeBtn.addEventListener("click", () => __awaiter(void 0, void 0, void 0, function* () {
            if (isLoggedIn) {
                const token = cookie.getCookie("session_token");
                const decodedToken = JSON.parse(atob(token.split('.')[1]));
                const userId = decodedToken.userid;
